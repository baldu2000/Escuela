...
class Nota3
{
    ...
    public static function combinar($materias, $notas3)
    {
        for ($x = 0; $x < count($materias); $x++) {
            $materias[$x]["puntaje"] = self::obtenerCalificacion($notas3, $materias[$x]["id"]);
        }
        return $materias;
    }

    private static function obtenerCalificacion($notas3, $idMateria)
    {
        foreach ($notas3 as $nota3) {
            if (intval($nota3["id_materia"]) === intval($idMateria)) {
                return $nota3["puntaje"];
            }
        }
        return 0;
    }
