...
class Notaoctubre
{
    ...
    public static function combinar($materias, $notasoctubre)
    {
        for ($x = 0; $x < count($materias); $x++) {
            $materias[$x]["puntaje"] = self::obtenerCalificacion($notasoctubre, $materias[$x]["id"]);
        }
        return $materias;
    }

    private static function obtenerCalificacion($notasoctubre, $idMateria)
    {
        foreach ($notasoctubre as $notasoctubre) {
            if (intval($notasoctubre["id_materia"]) === intval($idMateria)) {
                return $notasoctubre["puntaje"];
            }
        }
        return 0;
    }
