...
class Nota1
{
    ...
    public static function combinar($materias, $notas1)
    {
        for ($x = 0; $x < count($materias); $x++) {
            $materias[$x]["puntaje"] = self::obtenerCalificacion($notas1, $materias[$x]["id"]);
        }
        return $materias;
    }

    private static function obtenerCalificacion($notas1, $idMateria)
    {
        foreach ($notas1 as $nota1) {
            if (intval($nota1["id_materia"]) === intval($idMateria)) {
                return $nota1["puntaje"];
            }
        }
        return 0;
    }
