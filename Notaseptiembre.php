...
class Notaseptiembre
{
    ...
    public static function combinar($materias, $notasseptiembre)
    {
        for ($x = 0; $x < count($materias); $x++) {
            $materias[$x]["puntaje"] = self::obtenerCalificacion($notasseptiembre, $materias[$x]["id"]);
        }
        return $materias;
    }

    private static function obtenerCalificacion($notasseptiembre, $idMateria)
    {
        foreach ($notasseptiembre as $notasseptiembre) {
            if (intval($notasseptiembre["id_materia"]) === intval($idMateria)) {
                return $notasseptiembre["puntaje"];
            }
        }
        return 0;
    }
