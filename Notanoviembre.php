...
class Notanoviembre
{
    ...
    public static function combinar($materias, $notasnoviembre)
    {
        for ($x = 0; $x < count($materias); $x++) {
            $materias[$x]["puntaje"] = self::obtenerCalificacion($notasnoviembre, $materias[$x]["id"]);
        }
        return $materias;
    }

    private static function obtenerCalificacion($notasnoviembre, $idMateria)
    {
        foreach ($notasnoviembre as $notasnoviembre) {
            if (intval($notasnoviembre["id_materia"]) === intval($idMateria)) {
                return $notasnoviembre["puntaje"];
            }
        }
        return 0;
    }
